...
var resizePizzas = function(size) {
    ...
    function changePizzaSizes(size) {
        ...
        var slindingPizzasContainer = document.getElementsByClassName('randomPizzaContainer');
        // this 3 lines are better outside the loop
        var pizzasLength = slindingPizzasContainer.length;
        var dx = determineDx(slindingPizzasContainer[0], size);

        // this is the most critical (offsetWidth) is very time consuming
        var newwidth = (slindingPizzasContainer[0].offsetWidth + dx) + 'px';
        for (var i = 0; i < pizzasLength; i++) {
            slindingPizzasContainer[i].style.width = newwidth;
...
var pizzasDiv = document.getElementById("randomPizzas"); // I think is better to select pizzasDiv once only
for (var i = 2; i < 100; i++) {
...
function updatePositions() {
    ...
    // calling document.body.scrollTop is unnecesary on every iteration
	var cachedScrollTop = document.body.scrollTop;
    var i, phase = [];

    // pre-calculate unique 5 phase values
    for (i = 0; i < 5; i++) {
...
document.addEventListener('DOMContentLoaded', function() {
	var cols = 8;
	var s = 256;
    // get the number of pizzas needed in order to fill the browser window
    var pizzasNumber = Math.round(window.innerHeight / s) * cols;
    // cache the DOM selector
    var selector = document.getElementById('movingPizzas1');
